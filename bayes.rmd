...
\vspace*{\fill}
  \begin{center}
    \textbf{\Large{Program leave-one-out pointwise predictive checking in R}}%
  \end{center}
\vfill
...
Having the full conditionals we can apply the Gibbs sampling method to get draws from the conditional posteriors and to derive the expected values for $\beta$ and $\sigma^2$ as Bayesian estimates.
...
Indeed, both indicators favor model $9$. We now that according to [@gelman2014bayesian], *mse* is just appropriate if the model is more or less normally distributed. Furthermore if the models are normal and have constant variances, *mse* and *lppd* should have the same proportion.
